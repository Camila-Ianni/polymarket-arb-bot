...
class PreSignedBundle:
    ...
    signed_yes: dict   # payload listo para post_order
    signed_no: dict
    created_at: float
    presign_price_yes: float  # precio al que se pre-firmó
...
class ExecutionEngine:
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        """Sesión HTTP persistente con keep-alive. Se crea una sola vez."""
    ...
    def _presign_orders(self, ctx: MarketContext) -> PreSignedBundle:
        """
        Pre-firma ambas órdenes (YES/NO) en el hot path.
        Este método es SÍNCRONO y se llama desde un thread.
        """
        ...
        buy_price_yes = min(current_price_yes + SLIPPAGE_TOLERANCE, 0.99)
        buy_price_no  = min(current_price_no  + SLIPPAGE_TOLERANCE, 0.99)
        
        # Prevent division by zero
        buy_price_yes = max(buy_price_yes, 0.01)
        buy_price_no = max(buy_price_no, 0.01)
    ...
    async def _fire_order(self, ctx: MarketContext):
        """
        HOT PATH: solo HTTP POST con orden pre-firmada.
        """
        if self._fired:
            return
        ...
        logger.info(
            f"[FIRE] T-{remaining:.2f}s | "
            f"Disparando {outcome} @ last_price={ctx.last_price:.2f}"
        )
        ...
        try:
            ...
            if resp and resp.get("success"):
                logger.info(
                    f"[FILL] ✓ Order ID={resp.get('orderID')} | "
                    f"HTTP round-trip={elapsed_ms:.1f}ms"
                )
            else:
    ...
    def reset(self):
        """Limpiar estado entre ciclos de 5 minutos."""
        self._bundle    = None
        self._fired     = False
        self._presigned = False
    ...
    async def on_price_tick(self, ctx: MarketContext, price: float):
        """
        Callback principal — invocado en cada tick de Chainlink.
        Gestiona pre-firma e disparo basado en segundos restantes.
        """
        ...
        # ── Fase 1: Pre-firma (ventana idle)
        if not self._presigned and remaining <= PRESIGN_AT_SECONDS_REMAINING:
            self._presigned = True
            logger.info(f"[PRESIGN] T-{remaining:.0f}s → pre-firmando órdenes...")
            loop = asyncio.get_event_loop()
            self._bundle = await loop.run_in_executor(
                None, self._presign_orders, ctx
            )

        # ── Fase 2: Re-firma si el precio se movió demasiado
        elif (
            self._presigned
            and not self._fired
